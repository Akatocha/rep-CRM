...
div
  ...
  table
    ...
    tbody(v-if="lengthDataList")
      tr(v-for="item in USERS_API_DATA" :key="item.id")
        td  {{item.id}}
        td 
          v-icon(color="#F64E60" v-if="!item.online" ) mdi-close
          v-icon(color="#74EF60" v-if="item.online" ) mdi-check
        td 
          v-icon(color="#F64E60" v-if="!item.active" ) mdi-close
          v-icon(color="#74EF60" v-if="item.active" ) mdi-check
        td  {{item.roles[0].title}} 
  ...
  .table-footer
    .tf-left(v-if="lengthPagination")
      .tf-select
        ...
        .tf-drop-menu(v-if="showDropMenu")
          ...
          .drop-list(@click="pickDropButton(PAGIN_USERS_API.total)") All
    ...
    .tf-right
      .btn_add_new(@click="addBrokerDialog = true" ) Add User
      .tfr-input-box
...
computed: {
  ...
  lengthPagination() {
    if (this.PAGIN_USERS_API) {
      if (this.PAGIN_USERS_API.total == 0) {
        return false;
      } else {
        return true;
      }
  ...
  paginationNumbers() {
    ...
    let l = this.PAGIN_USERS_API.lastPage;
    if (l < 9) {
      let minPageArr = [];
      for (let x = 1; x <= l; x++) {
        minPageArr.push(x);
...
methods: {
  ...
  clickPaginationArrow(rl) {
    if (rl == "left" && this.currentPage > 1) {
      ...
      rl == "right" &&
      this.currentPage < this.PAGIN_USERS_API.lastPage
    ) {
      this.currentPage = this.currentPage + 1;
      this.refreshDataTable()
    }
  ...
  takeInputNumber() {
    ...
    if (x != NaN && x > 0) {
      if (x > this.PAGIN_USERS_API.total) {
        this.listCountInTable = this.PAGIN_USERS_API.total;
        this.refreshDataTable()
      } else {
