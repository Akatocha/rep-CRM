  ...
  .dialog_box
    ...
    .dialog_body
      .name_box
        .input_container
          .label First Name
          input.input_limit(v-model="item.first_name")
        .input_container
          .label Last Name
          input.input_limit(v-model="item.last_name")
      .input_container
        .label Email
        input.input_limit(v-model="item.email")
      .input_container(v-click-outside='closeDropRole')
        .label Role
        .input.check_one 
          .tag_area(@click.self="showDropRole")
            span {{roleSelectName}}
          v-icon(@click="showDropRole" :class="{ 'reverse_icon': dropRole}") mdi-chevron-down
        .drop_menu_box(v-if='dropRole') 
          .drop_row.title_select(v-for='item in PERMISSIONS_LIST'  :key='item.id' @click="takeRole(item)")
            span {{item.title}}
      //- .input_container(v-click-outside='closeDropSource')
      //-   .label Source
      //-   .input.check_one 
      //-     .tag_area(@click.self="showDropSource")
      //-       span {{sourceSelectName}}
      //-     v-icon(@click="showDropSource" :class="{ 'reverse_icon': dropSource}") mdi-chevron-down
      //-   .drop_menu_box(v-if='dropSource') 
      //-     .drop_row.title_select(v-for='item in GET_SOURCES'  :key='item.id' @click="takeSource(item)")
      //-       span {{item.title}}
      .input_container
        .checkbox_row
          .check_box
...
computed: {
    ...mapGetters([ 'ALL_PERMISSIONS', 'PERMISSIONS_LIST', 'GET_SOURCES'])
},
methods: {
  ...mapActions(['ShowAlert', 'SwitchPermissionStatus', 'AddAllPermissionStatus', 'RemoveAllPermissionStatus']),
  takeSource(item){
    this.sourceSelectName = item.title
    this.sourceSelectId = item.id
    this.dropSource = false
  },
  takeRole(item){
    this.roleSelectName = item.title
    this.roleSelectId = item.id
    this.dropRole = false
  },
  showDropSource(){
  ...
  sendData(){
    ...
    let config = {
        method: 'put',
        url: `https://back.horus.itdepartment.space/users/${this.item.id}`,
        headers: { 
          'Auth-Token': token, 
    ...
    if(this.item.first_name && this.item.last_name && this.item.email){
      this.axios(config)
          .then((response) => {
...
watch: {
  item(newValue, oldValue) {
    for (let i = 0; i < this.ALL_PERMISSIONS.length; i++) {
      ...
      let status = this.item.roles[0].permissions[objAll.title]
      objAll.status = status
    }
    this.roleSelectId = this.item.roles[0].id
    this.roleSelectName = this.item.roles[0].title
