  ...
  methods: {
    ...
    sendData(){
      if(this.limitInp){
        ...
        if(this.GET_COUNTRIES_SELECT.length>0 && this.GET_BROKERS_SELECT.length>0 && this.GET_SOURCES_SELECT.length>0){
          ...
          let config = {
              method: 'post',
              url: 'https://back.horus.itdepartment.space/rules/sources',
              headers: { 
                'Auth-Token': token,
