...
.input_container(v-click-outside='onClickOutside')
  ...
  .input(@click.self='showDropdownMenu = !showDropdownMenu')
    .tag_area
      ...
      input#inp_source.outline_none.search(@click='showDropdownMenu = !showDropdownMenu' @input='showDropdownMenu = true' @change='sortDataFromInput()' @keyup.enter="inputData = ''" v-model='inputData' type='text' autocomplete="off")
...
import {mapGetters, mapActions, mapMutations} from 'vuex'
  ...
  export default {
    ...
    computed: {
      ...mapGetters(['GET_SOURCES']),
      abrArrayPositiv() {
        let arr = this.GET_SOURCES.filter(c => c.status === true)
        return arr
      },
      abrArrayNegativ() {
        let arr = this.GET_SOURCES.filter(c => c.status === false)
        return arr
      },
      abrArrayReal(){
        if(this.reverseMode){
    ...
    methods: {
      ...mapActions(['ChangeSourceStatus', 'RemoveAllSourceStatus', 'AddAllSourceStatus']),
      onClickOutside(){
      ...
      sortDataFromInput(){
        if (this.inputData == ''){
          ...
          return this.GET_SOURCES.filter(c => c.title.toUpperCase().includes(this.inputData.toUpperCase()))
        }
      },
      clearAll(){
        if (!this.reverseMode){
      ...
      checkCountry(country) {
        let x = this.GET_SOURCES.indexOf(country)
        // console.log(x)
        this.ChangeSourceStatus(x)
        this.inputData = ''
        let inp = document.getElementById('inp_source')
        inp.focus()
      },
    },
    created () {
      ...
      setTimeout(()=>{
        if(typeof(this.$route.query.country)  == 'string' && this.GET_SOURCES.length>0){
          let element = this.$route.query.country
          let country = this.GET_SOURCES.find(item => item.id == element)
            let x = this.GET_SOURCES.indexOf(country)
            // console.log(x)
            this.ChangeSourceStatus(x)
        }
        if(typeof(this.$route.query.country)  == 'object' && this.GET_SOURCES.length>0){
